...
class SoftTeacher(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SoftTeacher, self).__init__(
            dict(student=build_detector(model),student2=build_detector(model)),
            train_cfg=train_cfg,
            test_cfg=test_cfg,
        )
        self.sign=0
  ...
  #          torch.save(loss_kl,"loss.pth")
    ...
    def extract_teacher_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        if proposals is None:
            ...
            rpn_out = list(self.student.rpn_head(feat))
            proposal_list = self.student.rpn_head.get_bboxes(
        ...
        proposal_list, proposal_label_list = self.student.roi_head.simple_test_bboxes(
        ...
        for img_id in range(len(proposal_list)):                                                                                 
            _bbox,_label,_inds=multiclass_nms(proposal_list[img_id],                                                             
                       proposal_label_list[img_id],self.student.test_cfg.rcnn.score_thr,                                          
            self.student.test_cfg.rcnn.nms,self.student.test_cfg.rcnn.max_per_img,return_inds=True)                              
            bbox.append(_bbox)                                                                                                   
            label.append(_label)                                                                                                 
            inds.append(_inds)                                                                                                   
    ...
    def extract_teacher2_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        if proposals is None:
            ...
            rpn_out = list(self.student2.rpn_head(feat))
            proposal_list = self.student2.rpn_head.get_bboxes(
                *rpn_out, img_metas=img_metas, cfg=proposal_cfg
            )
        else:
        ...
        proposal_list, proposal_label_list = self.student2.roi_head.simple_test_bboxes(
            feat, img_metas, proposal_list, None, rescale=False
        )
        ...
        for img_id in range(len(proposal_list)):                                                                                 
            _bbox,_label,_inds=multiclass_nms(proposal_list[img_id],                                                             
                       proposal_label_list[img_id],self.student2.test_cfg.rcnn.score_thr,                                          
            self.student2.test_cfg.rcnn.nms,self.student2.test_cfg.rcnn.max_per_img,return_inds=True)                              
            bbox.append(_bbox)                                                                                                   
            label.append(_label)                                                                                                 
            inds.append(_inds)                                                                                                   
    ...
    def compute_uncertainty_with_aug(
        ...
        bboxes, _ = self.student.roi_head.simple_test_bboxes(
            feat,
            img_metas,
            auged_proposal_list,
            None,
    ...
    def compute_uncertainty_with_aug2(
        ...
        bboxes, _ = self.student2.roi_head.simple_test_bboxes(
            feat,
            img_metas,
            auged_proposal_list,
            None,
