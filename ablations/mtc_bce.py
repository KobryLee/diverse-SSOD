...
class SoftTeacher(MultiSteamDetector):
    ...
    def unsup_rcnn_cls_loss(
           ...
           # print(len(gt_bboxes2[ind]))
            for jind,j in enumerate(i.pos_assigned_gt_inds):
                if gt_bboxes2[ind][j][4]<0.9:
                    ...
                    teacher_soft=torch.cat([teacher_soft,gt_bboxes2[ind][j][5:86].unsqueeze(0)],dim=0)
      ...
      #  print(teacher_soft)
        ...
        student_bg=torch.softmax(student_soft,dim=-1)[:,-1]
        teacher_bg=teacher_soft[:,-1]
        loss_bce=  teacher_bg*torch.log(student_bg)+(1-teacher_bg)*torch.log(1-student_bg)
        
#         bce = torch.nn.BCELoss(reduction="mean")
#         loss_bce = torch.nn.BCELoss(torch.softmax(student_soft,dim=-1)[:,-1],teacher_soft[:,-1])
        
#         loss_kl = F.kl_div(torch.log_softmax(student_soft,dim=1),teacher_soft,reduction="none")
        if self.sign==0:
            torch.save(loss_bce,"loss.pth")
       # if self.sign%200==0:
           # print(loss_kl)
           # print(ignore_num)
      #  print(ignore_num)
